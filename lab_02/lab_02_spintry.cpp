...
pthread_spinlock_t global; //для проверки работоспособности ошибки Device or resource busy
...
void* proc1(void* arg) {
    ...
    while (curArgs->flag != 1) {
        int ret_val = pthread_spin_trylock(&global_spin);
        if (!ret_val) {
            ...
            std::cout << '\n' << "ошибка в первом потоке: " << strerror(ret_val) << '\n';
        sleep(1);
        }
    }
...
void* proc2(void* arg) {
    ...
    while (curArgs->flag != 1) {
        ...
        if (!ret_val) {
            ...
            std::cout << '\n' << "ошибка во втором потоке: " << strerror(ret_val) << '\n';
            sleep(1);
        }
    }
